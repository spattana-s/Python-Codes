...
dfdf = pd.DataFrame(df, columns=['SampleSize','AvgF1SVM'])
print("SVM Classifier")
print(dfdf)
...
dfdf1 = pd.DataFrame(df1, columns=['SampleSize','AvgF1NaiveBayes'])
print("Naive Bayes Classifier\n")
print(dfdf1)
...
final_dataset = pd.merge(dfdf,dfdf1, how = 'inner', on = ['SampleSize'])
...
plt.plot(dfdf.SampleSize,dfdf.AvgF1SVM);
...
plt.plot(dfdf1.SampleSize,dfdf1.AvgF1NaiveBayes);


# In[ ]:


final_dataset.groupby('SampleSize')['AvgF1SVM','AvgF1NaiveBayes'].mean().plot(kind='line', figsize=(15,5)).legend(loc='center left', bbox_to_anchor=(1, 0.5));  # set legend
plt.title('Comparision of SVM and NaiveBayes Cassifier')
plt.xlabel('Sample Size', fontsize=16)
plt.ylabel('AvgF1', fontsize=16)


# In[ ]:


grouped=final_dataset.groupby(['AvgF1SVM','AvgF1NaiveBayes'])
print(grouped.std())
...
print("Variance of SVM Classifier\n")
print(np.std(final_dataset.AvgF1SVM))


# In[ ]:


print("Variance of Naive Bayes Classifier\n")
print(np.std(final_dataset.AvgF1NaiveBayes))
